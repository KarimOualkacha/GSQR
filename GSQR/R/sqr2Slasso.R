sqr2Slasso <-
function(alpha,taux, k, bn, bs, ix, iy, gamma, nobs, nvars, x, y, 
                      pf, dfmax, pmax, nlam, ulam, eps, maxit, vnames, group, intr) {
  #################################################################################
  # call Fortran core
  gamma <- 2 * gamma/(k * nobs)
  gamma <- as.double(gamma)
  fit <- .Fortran("sqr2Slasso",alpha, taux, k, bn, bs, ix, iy, gamma, nobs, nvars, as.double(x), 
                  as.double(y), pf, dfmax, pmax, nlam, ulam, eps, maxit, intr, nalam = integer(1), 
